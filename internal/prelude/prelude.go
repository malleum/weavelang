...
var Values = []Entry{
	...
	{"under", "Earth -> Thread Earth", "", "every whole number from zero up to but not including n", "Sequences"},
	{"copies", "Earth -> a -> Thread a", "", "a Thread of n of the same value; `repeat` lays one Thread end to end instead", "Sequences"},
	{"flow", "(a -> a) -> a -> Thread a", "", "the endless Thread seed, f seed, f (f seed), ...", "Sequences"},
	{"settle", "(a -> a) -> a -> a", "Eq a", "apply until a round changes nothing, and answer that", "Sequences"},
	...
	{"take", "Earth -> a -> a", "Ply a", "the first n elements, or the first n characters of text", "Sequences"},
	{"drop", "Earth -> a -> a", "Ply a", "everything after the first n elements, or characters", "Sequences"},
	...
	{"weld", "a -> a -> a", "Ply a", "weld ys xs: xs with ys on the end, for a Thread or for text", "Sequences"},
	{"turn", "Earth -> a -> a", "Ply a", "turn n xs: the same elements shifted round n places, the front going to the back", "Sequences"},
	{"wrap", "Earth -> Thread a -> Hold a", "", "wrap i xs: `nth` that goes round, so -1 is the last and len is the first", "Sequences"},
	{"wind", "Earth -> a -> Thread a -> Thread a", "", "wind i x xs: `mend` that goes round, so -1 replaces the last strand", "Sequences"},
	{"mend", "Earth -> a -> Thread a -> Thread a", "", "mend i x xs: xs with position i replaced, or xs when there is no such position", "Sequences"},
	{"sever", "Earth -> a -> (a, a)", "Ply a", "cut a Thread or some text in two at a position", "Sequences"},
	...
	{"first", "c -> Hold e", "Strand c e", "the first element, or the first character of text", "Sequences"},
	{"second", "Thread a -> Hold a", "", "the second element, if there is one", "Sequences"},
	{"last", "c -> Hold e", "Strand c e", "the final element, or the last character of text", "Sequences"},
	{"rev", "a -> a", "Ply a", "the same elements, or characters, back to front", "Sequences"},
	...
	{"priors", "(b -> a -> b) -> b -> Thread a -> Thread b", "", "scan with the starting value in front, so one longer than the Thread", "Sequences"},
	{"gentle", "(b -> a -> Weaving b c) -> b -> Thread a -> Weaving b c", "", "braid that stops when the step answers Gentled", "Sequences"},
	{"dupe", "Thread a -> Hold (Earth, Earth, a)", "Eq a", "where the first repeat is, where it was before, and what it is", "Sequences"},
	...
	{"siftidx", "(a -> Spirit) -> Thread a -> Thread Earth", "", "where every element satisfying a test is", "Sequences"},
	{"idxs", "a -> Thread a -> Thread Earth", "Eq a", "every position a value occurs at", "Sequences"},
	{"index", "Thread a -> Web a Earth", "Eq a", "where each value sits, the first when it repeats; `freq` counts them instead", "Sequences"},
	{"squeeze", "Thread Earth -> Thread Earth", "", "a sparse axis made dense: the sorted distinct values, plus one stand-in per gap", "Sequences"},
	...
	{"mesh", "Thread (Earth, Earth) -> Thread (Earth, Earth)", "", "overlapping and touching ranges merged, in order", "Ranges"},
	...
	{"pairs", "Thread a -> Thread (a, a)", "", "each element paired with the *next* one; `couples` pairs it with every later one", "Sequences"},
	{"couples", "Thread a -> Thread (a, a)", "", "each element paired with *every* element after it; `pairs` takes only the next one", "Sequences"},
	...
	{"nth", "Earth -> c -> Hold e", "Strand c e", "the element at a position, or the character at one in text", "Sequences"},
	...
	{"freq", "Thread a -> Web a Earth", "Eq a", "count how often each element occurs; `index` says where each one is instead", "Sequences"},
	{"most", "Web a Earth -> Hold a", "", "the key with the highest count", "Maps"},
	{"contains", "Air -> Air -> Spirit", "", "contains needle haystack: does the text hold this run, unbroken", "Text"},
	{"earths", "Air -> Thread Earth", "", "every Earth appearing in some text; a `-` after a digit is read as a sign, so use `spans` for `11-22`", "Text"},
	{"spans", "Air -> Thread (Earth, Earth)", "", "every inclusive range `11-22` appearing in some text", "Text"},
	{"waters", "Air -> Thread Water", "", "every Water appearing in some text", "Text"},

	// Text
	{"lines", "Air -> Thread Air", "", "split text into lines", "Text"},
	{"words", "Air -> Thread Air", "", "split text on whitespace", "Text"},
	{"carve", "Air -> Air -> Thread Air", "", "carve seps text: the runs between any of these characters, empty runs dropped", "Text"},
	...
	{"repeat", "Earth -> a -> a", "Ply a", "repeat n xs: a Thread or some text laid end to end n times", "Sequences"},
	{"base", "Earth -> Earth -> Air", "", "base b n: the spelling of an Earth in a base from 2 to 36", "Text"},
	{"unbase", "Earth -> Air -> Hold Earth", "", "unbase b text: read an Earth written in a base from 2 to 36", "Text"},
	...
	{"woven", "Weaving a e -> Spirit", "", "did this Weaving come back Woven rather than Gentled", "Absence and failure"},
	...
	{"warp", "(Knot -> a) -> Earth -> Earth -> Pattern a", "", "warp f rows cols: a grid of that shape, each cell worked out from its knot", "Grids"},
	...
	{"tallies", "Pattern a -> Pattern a", "Reckon a", "each cell replaced by the total of the box from the top left corner to it", "Grids"},
	{"tallied", "Pattern a -> Knot -> Knot -> a", "Reckon a", "the total over an inclusive box, read out of a grid `tallies` made", "Grids"},
	{"cells", "Pattern a -> Thread a", "", "every cell of a grid", "Grids"},
	{"sited", "Pattern a -> a -> Hold Knot", "Eq a", "where this value first sits in the grid, in reading order", "Grids"},
	{"sites", "Pattern a -> a -> Thread Knot", "Eq a", "every knot holding this value, in reading order", "Grids"},
	...
	{"covers", "Circle a -> Circle a -> Spirit", "Eq a", "covers outer inner: is every member of the second also in the first", "Sets"},
	...
	{"clumps", "(a -> Thread a) -> Thread a -> Thread (Thread a)", "Eq a", "the nodes in groups that can reach one another, given a step function", "Priority queues and graphs"},
	{"link", "Thread a -> Link a", "Eq a", "these nodes, each in a circle of its own", "Priority queues and graphs"},
	{"bind", "Link a -> a -> a -> Link a", "Eq a", "join the circles holding these two", "Priority queues and graphs"},
	{"bound", "Link a -> a -> a -> Spirit", "Eq a", "are these two in one circle yet", "Priority queues and graphs"},
	{"clumped", "Link a -> Thread (Thread a)", "", "the circles, each once, in the order their first member was given", "Priority queues and graphs"},
	...
	{"solve", "Pattern Earth -> Weaving (Thread Earth) (Thread Earth, Thread (Thread Earth))", "", "the whole-number solutions of an augmented matrix: Woven when one, Gentled with a point and the directions when many", "Numbers"},
